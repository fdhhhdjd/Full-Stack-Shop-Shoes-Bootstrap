  ...
  useEffect(() => {
    if (result_product?.length > 0) {
      result_product?.map((item) => {
        if (
          item?.categories === result_product_detail.product_detail[0]?.categories &&
                ...
                return (
                  <Link to={`/product/${relation._id}`} key={index}>
                    <div className="product-card">
                      <img src={relation.image.url} width={250} height={250} className="product-image" alt="img" />
                      <p className="product-name123">{STORAGES.except(relation.name, 15)}</p>
